...
test('parses the window list', () => {
  const out = '@1\tzsh\t12\t1\t\t\t\tshell\n@2\tssh\t34\t0\taltair\tmy-project\talways\taltair\n'
  assert.deepStrictEqual(parseWindows(out), [
    { id: '@1', command: 'zsh', pid: '12', autoRename: true, marker: '', prev: '', mode: '', name: 'shell' },
    {
      id: '@2', command: 'ssh', pid: '34', autoRename: false,
      marker: 'altair', prev: 'my-project', mode: 'always', name: 'altair',
    },
  ])
})
...
test('a window whose command contains a space stays intact', () => {
  assert.deepStrictEqual(parseWindows('@3\tmy helper\t56\t1\t\t\t\tshell\n'), [
    {
      id: '@3', command: 'my helper', pid: '56', autoRename: true,
      marker: '', prev: '', mode: '', name: 'shell',
    },
  ])
})
...
test('a malformed window row keeps its id so its options still get written', () => {
  assert.deepStrictEqual(parseWindows('@4\n'), [
    { id: '@4', command: null, pid: null, autoRename: false, marker: '', prev: '', mode: '', name: '' },
  ])
})
...
// Reading the option as a field keeps the ticker at one tmux invocation, which
// is the constraint the whole design exists to satisfy.
test('the window format carries the opt-in and the previous name', () => {
  assert.ok(WINDOW_FORMAT.includes('#{@tmarchy-ssh-rename}'))
  assert.ok(WINDOW_FORMAT.includes('#{@tmarchy-ssh-prev}'))
  // window_name stays last: it is the only field that may contain a tab.
  assert.ok(WINDOW_FORMAT.endsWith('#{window_name}'))
})
