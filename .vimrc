...
" Follow tmarchy's theme instead of naming one. The scheme comes from
" ~/.local/state/tmarchy/current.lua, generated by `tmarchy-theme sync` from
" the same themes/<name>.conf the tmux bar reads. Falls back to
" tokyonight-night on a host that has never run tmarchy, so this is safe on a
" fresh checkout. :TmarchyTheme repaints a running editor after a switch.
lua require('tmarchy-theme').setup()
lua require('tmarchy-theme').apply({ quiet = true })
