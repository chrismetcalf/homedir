...
| Moving nvim off `spaceduck` | Real scope (colorscheme plugin, lualine theme, indent-blankline colors). Until it happens, nvim will clash with a Tokyo Night bar. Accepted knowingly. **Delivered**: each `tmarchy/themes/<name>.conf` now declares `@theme-nvim`/`@theme-bat` alongside its bar colours; `tmarchy-theme sync` writes `~/.local/state/tmarchy/current.lua`, which `.vim/lua/tmarchy-theme.lua` reads to apply the matching colorscheme (falling back to `tokyonight-night` if the state is missing or the scheme isn't installed) — nvim no longer clashes with the bar. lualine's `theme = 'auto'` follows the same switch rather than pinning a palette. |
