...
| Keybinding overhaul | Deliberately last — it re-labels everything the other pieces add, and the menu should be regenerated from the final scheme rather than edited twice. **Delivered**: the which-key menu regrouped from tmux nouns into ten verbs (`+Go`/`+Open`/`+Move`/`+Set`/`+Copy`/`+New`/`+Ask`/`+Show`/`+Kill`) plus one deliberate noun exception, `+Agents`; `bin/tmux-cmd` now takes a row's glyph from its declared verb instead of guessing from command text; `bin/tmux-menu-coverage` asserts the rewrite lost no command. Spec `2026-09-05-tmux-keybinding-overhaul-design.md`. |
| Moving nvim off `spaceduck` | Real scope (colorscheme plugin, lualine theme, indent-blankline colors). That clash with a Tokyo Night bar was accepted knowingly until it was fixed. **Delivered**: each `tmarchy/themes/<name>.conf` now declares `@theme-nvim`/`@theme-bat` alongside its bar colours; `tmarchy-theme sync` writes `~/.local/state/tmarchy/current.lua`, which `.vim/lua/tmarchy-theme.lua` reads to apply the matching colorscheme (falling back to `tokyonight-night` if the state is missing or the scheme isn't installed) — nvim no longer clashes with the bar. lualine's `theme = 'auto'` follows the same switch rather than pinning a palette. |
