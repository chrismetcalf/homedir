...
- **`tmarchy-saver`** (+ **`tmarchy-geo.js`**): the screensaver, on `prefix + Z` and in `+Open` — two renderers sharing one frame loop, `geo` (default: a rotating solid) and `rain`. Both paint into the same character grid, so the agent nodes and HUD are written once and neither renderer knows they exist. Rain, a node row and a HUD, painted in the **active tmarchy theme** — it reads `~/.local/state/tmarchy/current.sh` rather than live tmux options, for the same reason `tmarchy-theme sync` writes that file: it has to work with no server running, and switching theme moves the rain on the next data tick. Everything on screen is real: the nodes are live agent panes coloured by `sessionState()` — the same single classifier the bar, `prefix + ~` and `tmux-agents` use, so the dots cannot disagree with the tab colours above them — and the readouts are load, agent counts and the five-hour quota.
  ...
  - **`tmarchy-panel.js` is the right-hand telemetry column** — CPU, memory, disk, network, temperature and load from `/proc` and `statfsSync`, the live agent list, and the quota gauge, with blinking lamps. Every lamp is driven by something real: NET flickers because bytes actually moved. Decorative blinking would be easier and would also be a lie, next to numbers you might act on. CPU and network are **rates**, so they are sampled on the 2s data tick rather than per frame — per frame the deltas would be noise divided by 50ms. Network sums **physical interfaces only**: a box running containers has a dozen `veth`/`docker`/`br-` devices whose counters double-count the real NIC. The panel owns its columns and everything else centres in what is left; below 74 columns it disappears and the pane belongs to the solid.
  ...
  - **Text lives at the edges, not the middle.** Nodes hug the top row and the HUD the bottom. With rain that hardly mattered — rain is a backdrop — but text through the centre punches a hole in a solid and takes the depth illusion with it, so the solid is centred in what is *left* of the pane, not the pane.
