...
- **`tmarchy-saver`** (+ **`tmarchy-geo.js`**): the screensaver, on `prefix + Z` and in `+Open` — two renderers sharing one frame loop, `geo` (default: a rotating solid) and `rain`. Both paint into the same character grid, so the telemetry panel and the CPU graph are written once and neither renderer knows they exist. Painted in the **active tmarchy theme** — it reads `~/.local/state/tmarchy/current.sh` rather than live tmux options, for the same reason `tmarchy-theme sync` writes that file: it has to work with no server running, and switching theme moves the rain on the next data tick. Everything on screen is real: the nodes are live agent panes coloured by `sessionState()` — the same single classifier the bar, `prefix + ~` and `tmux-agents` use, so the dots cannot disagree with the tab colours above them — and the readouts are load, agent counts and the five-hour quota. The pane is divided once per frame: the panel takes the right columns, the CPU graph the bottom band, and the solid centres in what is left.
  ...
  - **The CPU graph is a full-width bottom band** (`panel.renderGraph`): CPU on the Y axis, time on the X, newest sample at the right edge. It is drawn *before* the panel, so where the two overlap the panel wins and the timeline reads as a backdrop running under it rather than as something clipped at the panel's edge. A shorter history starts further in rather than being stretched, so the time axis keeps a constant scale. The Y axis is a fixed 0–100 — an autoscaled one would make an idle machine look busy — which is why an idle box shows little more than the baseline row, and that is the intended "subtle". Sub-cell resolution comes from the eighth-block characters (`▁`–`█`), so a 4-row band still resolves 32 levels. Below 18 rows the band is dropped entirely rather than squeezed.
  ...
  - `lock-command` is set to it, so `lock-server` and any idle lock use the same screen. **`lock-after` is deliberately left at 0**: pairing them would start locking the session on a timer, which is a behaviour change nobody asked for. Set `lock-after <seconds>` yourself to make it idle-triggered.
