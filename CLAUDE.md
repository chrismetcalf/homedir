...
- **`tmux-goto`**: `prefix + u` popup listing sessions, windows and panes in one fzf picker — no category to choose first. Rows are tagged `[sess]`/`[win]`/`[pane]` so a query can be narrowed by typing the tag; pane rows show the running command (with worktree-named windows that is the recognisable part) and window rows carry tmarchy's `@scout-state` only when it is worth reacting to (`wait`/`busy`/`done` render as `<state>`; unset and `idle` render nothing, so the marked windows stand out). The target id is field 1 and hidden from display via fzf's `--with-nth=2..`, so a window name containing spaces cannot break the action, and `act_on` dispatches on the row's `[sess]`/`[win]`/`[pane]` tag rather than the target's first character — a session may legally be named `@release`. Navigation only by design: `prefix + F` keeps tmux-fzf's nine modes for commands, keybindings and processes.
