...
- **Keybindings**:
  ...
  - `prefix + B` / `prefix + e`: drop-down btop / lazydocker overlays (toggle — any dropdown key closes whichever is open)
...
- **`tmux-dropdown`**: `tmux-dropdown <_session> <command…>` — the mechanism behind `prefix + B` (btop) and `prefix + e` (lazydocker). Not `b`; that is `break-pane`. Each tool runs in its **own detached session** (`_btop`, `_docker`) rather than in the popup command, so the process survives between glances: a popup running the command directly would start a fresh one every toggle, losing whatever state it has built up — btop's history graphs, lazydocker's selected container — and paying startup each time. The popup only attaches a second client. The binding is a real toggle — `if-shell -F '#{m:_*,#{session_name}}'` detaches when you are already inside *any* overlay, which is the only coherent behaviour since a popup cannot nest inside a popup — and `-F` evaluates the format directly rather than forking a shell per keypress. The leading `_` is enforced by the script, not merely conventional: it is what both the toggle condition and the picker's filter key off. Two tmux quirks it works around: attaching from inside another client needs `TMUX=` cleared ("sessions should be nested with care"), and this tmux accepts the `=name` exact-match target for `has-session` but **rejects it for `set-option`**. A session whose name starts with `_` is treated as infrastructure and hidden from `tmux-goto`'s picker — it is somewhere you summon, not somewhere you navigate to.
