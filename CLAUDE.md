...
- **tmarchy** — native status bar (`tmarchy/`, replaced tmux-powerline 2026-08-25). Loaded by an explicit `run-shell` AFTER tpm rather than as a tpm plugin (tpm load ordering has caused three separate incidents in this repo). Four layers:
  ...
  3. **`tmarchy/bin/tmarchy-tick-guard`** (invoked by that `#()`) **+ `tmarchy/bin/tmarchy-tick`** — the ticker. tmux re-runs a `#()` on every status *redraw* once the previous job exits, not on `status-interval` — with a dozen busy agent panes, redraws are near-continuous, so an unguarded ticker measured 2.0 node spawns/sec. The guard is bash-builtins-only (no `date`/`cat`/test binary) and drops all but one call per `status-interval` (5s) **before node ever starts**; only a call that survives execs `tmarchy-tick`, which writes `@scout-state`/`@remote-host` per window and `@bar-*` globals (load, agents, branch, remote, battery) for `bar.conf` to read on the next redraw. A failed or skipped tick just leaves the previous values standing — one interval stale, never broken.
  ...
  - **Governing invariant: nothing forks a shell on a status redraw.** This is what took status-bar forks from 7.7/sec under powerline to 0.5/sec under tmarchy — the tick guard above is what makes it hold even with a dozen busy agent panes.
...
- **tmux-scout** (qeesung/tmux-scout): tracks Claude Code / Codex / Gemini / etc. sessions
  ...
  - `tmarchy/bin/tmarchy-tick` runs every status redraw (behind its rate guard, effectively every status-interval) and sets `@scout-state` per window so window-status-format tints the whole tab text: **red=wait, orange=busy, teal=done**, dim/none=idle
...
3. **A slow `tmarchy-tick`** — many transient `node` children under `tmux: server`, or the bar visibly stuttering. The tmux-powerline cause of this (its per-window `#()` callout resetting `status-interval` to 1 on every tpm load — 1,180 forks/sec at 73% idle CPU, the 2026-08-21 incident) is gone: tmarchy loads after tpm, not as a tpm plugin, so nothing resets its `status-interval 5`, and `tmarchy/bin/tmarchy-tick-guard` (bash builtins only, no `date`/`cat`/test binary) caps the ticker to one node spawn per interval *before* node ever starts — tmux still re-runs the status `#()` on every redraw, not on `status-interval`, but almost all of those calls now exit in the guard. So the check now is simply **whether the tick itself is slow**: `time tmarchy/bin/tmarchy-tick` (should be well under a second) and `ps -eo comm,etimes | grep -c tmarchy-tick` (should be 0 or 1 concurrent, never a pile-up). If it's genuinely slow, suspect a segment in `tmarchy/segments.d/` doing something expensive, not the guard.
